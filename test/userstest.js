...
describe('TEST FOR USER CLASS', function() {
    ...
    describe('MONGOOSE USER', function(){
        it('gets a user by name successfully', function(done){
        ...
        it('gets a user by email successfully', function(done){
        ...
        it('gets a user by role successfully', function(done){
        ...
        it('hash password successfully', function(done){
        ...
        it('null password returns false for hash password', function(done){
            let result = User.find({}).byName('testclient');
            result.then(user => {

                expect(user[0]).to.not.be.undefined;
                const result = user[0].generateHash();
        ...
        it('null password returns false', function(done){
            let result = User.find({}).byName('testclient');
            result.then(user => {

                expect(user[0]).to.not.be.undefined;
                const result = user[0].validPassword();
        ...
        it('null password does not validate pass validation check', function(done){
            let result = User.find({}).byName('testclient');
            result.then(user => {
        ...
        it('provider returns undefined when no provider is inserted', function(done){
            let result = User.find({}).byName('testclient');
            result.then(user => {
                expect(user[0]).to.not.be.undefined;
                expect(user[0].providers.length).to.be.equal(0);
                const result = user[0].getProvider('google');
                expect(result).to.be.undefined
                done();
            })
        });
        it('get provider returns undefined with invalid data', function(done){
            let result = User.find({}).byName('testclient');
            result.then(user => {
